import { HeartHandshake, Laptop, Recycle, ShieldCheck, TabletSmartphone, Watch } from "lucide-react";
...
const WhyUs = () => {
    return (
        <div className='flex justify-between items-centerm m-5 my-20 flex-wrap gap-10'>
            {whyUS.map((why, index) => (
                <div key={index} className='flex flex-col gap-3 justify-center items-center w-[320px] max-md:w-[100%] bg-[#FB6D11] p-20 rounded-lg'>
                    <div>{why.icon}</div>
                    <h1 className='text-center text-xl text-white  max-md:text-lg max-sm:text-sm font-bold'>{why.heading}</h1>
                    <p className='text-center text-lg max-sm:text-sm w-[250px] max-md:w-[100%] font-medium'>{why.description}</p>
                </div>
            ))}
        </div>
    )
